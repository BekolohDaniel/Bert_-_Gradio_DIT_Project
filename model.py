...
class CustomDataset(Dataset):
    def __init__(self, file_name="./Corona_NLP_train.csv", tokenizer_name="google-bert/bert-base-cased", max_length=128):
        self.df = pd.read_csv(file_name, encoding="latin1")
        self.df = self.df[["OriginalTweet", "Sentiment"]]
        self.text = self.df["OriginalTweet"].astype(str).tolist()
        self.label = self.df["Sentiment"].astype(str).tolist()
        self.max_length = max_length
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
        
        # We explicitly map all variations into 3 safe classes: 0, 1, or 2
    ...
    def __getitem__(self, idx):
        text = self.text[idx]
        label_str = str(self.label[idx]).lower().strip()
        ...
        # SUCCESSFUL STRATEGY: Map all 5 Corona NLP labels into 3 core targets
        if "negative" in label_str:
            target_label = self.label_id["negative"]
        elif "positive" in label_str:
            target_label = self.label_id["positive"]
        else:
            target_label = self.label_id["neutral"]
...
def main():
    ...
    torch.save(model.state_dict(), "./model_checkpoint/sentiment_model.pt")
    dataset.tokenizer.save_pretrained("./model_checkpoint")
    print("Saved model weights and tokenizer configurations to ./model_checkpoint")
